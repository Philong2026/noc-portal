...
import { Activity, AlertTriangle, Bell, Check, ChevronDown, Cloud, Cpu, Database, HardDrive, LayoutDashboard, LockKeyhole, LogOut, Menu, Moon, Network, Search, Server, Settings, ShieldCheck, Sun, UserPlus, Users, X, Zap } from 'lucide-react'
...
function Dashboard() {
  const location = useLocation(); const [dashboardData, setDashboardData] = useState(null); const [refreshing, setRefreshing] = useState(false)
  const refresh = () => { setRefreshing(true); mockApi.getDashboard().then(setDashboardData).finally(() => setRefreshing(false)) }
  useEffect(() => { refresh() }, [])
...
function MonitoringPage() { const [devices, setDevices] = useState([]); const [query, setQuery] = useState(''); const [status, setStatus] = useState('All'); const [selected, setSelected] = useState(null); useEffect(() => { mockApi.getDevices().then(data => { setDevices(data); setSelected(data[0]) }) }, []); const filtered = devices.filter(device => (status === 'All' || device.status === status) && `${device.name} ${device.id} ${device.type} ${device.location}`.toLowerCase().includes(query.toLowerCase())); return <div className="dashboard"><PageHeader eyebrow="Monitoring" title={<>Know what is<br /><span>happening now.</span></>} text="Live health signals from every monitored server, device, and network path." action="Refresh data" /><div className="metric-grid">{metrics.slice(0, 4).map(metric => <MetricCard key={metric.label} {...metric} />)}</div><NetworkMap /><div className="monitoring-layout"><section className="dashboard-card monitor-table"><CardHeader eyebrow="Monitored assets" title={`${filtered.length} devices online`} /><div className="monitor-toolbar"><div className="device-search"><Search size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search devices..." /></div><select value={status} onChange={event => setStatus(event.target.value)}><option>All</option><option>Operational</option><option>Degraded</option><option>Warning</option></select></div><div className="table-row table-head"><span>Asset</span><span>Type</span><span>Location</span><span>Health</span></div>{filtered.map(device => <button className={`table-row device-row ${selected?.id === device.id ? 'selected' : ''}`} key={device.id} onClick={() => setSelected(device)}><strong>{device.id}</strong><span>{device.type}</span><span>{device.location}</span><span className={`status-pill ${device.status === 'Operational' ? 'green' : 'amber'}`}>{device.status}</span></button>)}</section><DeviceDetail device={selected} /></div></div> }
function DeviceDetail({ device }) { if (!device) return <section className="dashboard-card device-detail"><span className="auth-eyebrow">Device detail</span><h2>Select a device</h2><p>Choose an asset from the inventory to inspect health and connectivity.</p></section>; return <section className="dashboard-card device-detail"><div className="detail-heading"><div><span className="auth-eyebrow">Device detail</span><h2>{device.name}</h2><p>{device.id} &middot; {device.ip}</p></div><span className={`status-pill ${device.status === 'Operational' ? 'green' : 'amber'}`}>{device.status}</span></div><div className="detail-stats"><span><strong>{device.uptime}</strong><small>Uptime</small></span><span><strong>{device.ping} ms</strong><small>Ping latency</small></span><span><strong>{device.lastCheck}</strong><small>Last check</small></span></div><div className="detail-bars"><UsageBar label="CPU" value={device.cpu} tone="cyan" /><UsageBar label="RAM" value={device.ram} tone="blue" /><UsageBar label="Disk" value={device.disk} tone="amber" /></div><button className="outline-button">Open device history</button></section> }
...
function AlertsPage() { const [items, setItems] = useState([]); const [severity, setSeverity] = useState('All'); useEffect(() => { mockApi.getAlerts().then(setItems) }, []); const acknowledge = id => setItems(current => current.map(item => item.id === id ? { ...item, status: 'Acknowledged' } : item)); const filtered = items.filter(item => severity === 'All' || item.severity === severity); const active = filtered.filter(item => item.status !== 'Acknowledged' && item.status !== 'Resolved'); return <div className="dashboard"><PageHeader eyebrow="Alerts" title={<>Resolve issues<br /><span>before impact.</span></>} text="Prioritized incidents and threshold events across the NOC estate." action="Acknowledge all" /><div className="alert-filters"><button className={severity === 'All' ? 'active' : ''} onClick={() => setSeverity('All')}>All <span>{items.length}</span></button>{['Critical', 'Warning', 'Information'].map(level => <button className={severity === level ? 'active' : ''} key={level} onClick={() => setSeverity(level)}>{level} <span>{items.filter(item => item.severity === level).length}</span></button>)}</div><div className="dashboard-grid"><section className="dashboard-card alerts-card"><CardHeader eyebrow="Current queue" title={`${active.length} active alerts`} /><div className="alert-list">{active.map(item => <div className="alert-row detailed-alert" key={item.id}><span className={`alert-icon ${item.severity.toLowerCase()}`}><AlertTriangle size={15} /></span><span><strong>{item.title}</strong><small>{item.device} &middot; {item.time}</small></span><button className="ack-button" onClick={() => acknowledge(item.id)}>Acknowledge</button></div>)}</div></section><section className="dashboard-card alert-health"><CardHeader eyebrow="Response health" title="Alert performance" /><div className="alert-stat"><strong>14 min</strong><span>mean time to acknowledge</span></div><div className="alert-stat"><strong>96.4%</strong><span>alerts resolved within SLA</span></div><div className="summary-note"><Check size={15} /> Response targets are on track</div></section></div><section className="dashboard-card timeline-card"><CardHeader eyebrow="Alert timeline" title="Recent activity" /><div className="alert-timeline">{items.map(item => <div key={item.id} className="timeline-entry"><span className={`timeline-marker ${item.severity.toLowerCase()}`} /><div><strong>{item.title}</strong><small>{item.device} &middot; {item.time}</small><span className="timeline-status">{item.status}</span></div></div>)}</div></section></div> }
function ReportsPage() { const [report, setReport] = useState(null); useEffect(() => { mockApi.getReports().then(setReport) }, []); const download = (period, format) => { const blob = new Blob([`NOC Automation ${period} report\nAvailability: ${report?.monthly || 99.98}%\nIncidents resolved: ${report?.incidents || 38}`], { type: format === 'CSV' ? 'text/csv' : 'application/pdf' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `noc-automation-${period.toLowerCase()}-report.${format === 'CSV' ? 'csv' : 'pdf'}`; link.click(); URL.revokeObjectURL(url) }; return <div className="dashboard"><PageHeader eyebrow="Reports" title={<>Operational clarity<br /><span>on demand.</span></>} text="Shareable performance summaries for infrastructure, availability, and response." action="Generate report" /><div className="report-grid"><ReportCard period="Daily" value={report ? '99.98%' : '—'} detail="Availability across the last 24 hours" onDownload={() => download('Daily', 'PDF')} /><ReportCard period="Weekly" value={report ? '99.96%' : '—'} detail="Availability across the last 7 days" onDownload={() => download('Weekly', 'CSV')} /><ReportCard period="Monthly" value={report ? '99.98%' : '—'} detail="Availability across the last 30 days" onDownload={() => download('Monthly', 'PDF')} /></div><section className="dashboard-card report-chart"><CardHeader eyebrow="Monthly trend" title="Service availability" /><div className="report-bars">{[92, 94, 91, 96, 95, 98, 97, 99, 98, 99, 99, 100].map((height, index) => <span key={index} style={{ height: `${height}%` }} />)}</div><div className="chart-axis"><span>Oct</span><span>Dec</span><span>Feb</span><span>Apr</span><span>Jun</span><span>Sep</span></div></section></div> }
...
function SettingsPage() { const [settings, setSettings] = useState(null); const [tab, setTab] = useState('profile'); const [saved, setSaved] = useState(false); useEffect(() => { mockApi.getSettings().then(setSettings) }, []); if (!settings) return <div className="dashboard"><PageHeader eyebrow="Settings" title="Loading settings" text="Retrieving workspace configuration." /></div>; return <div className="dashboard"><PageHeader eyebrow="Settings" title={<>Tune the<br /><span>operations center.</span></>} text="Configure monitoring preferences, notification routing, and workspace roles." /><div className="settings-tabs"><button className={tab === 'profile' ? 'active' : ''} onClick={() => setTab('profile')}>User Profile</button><button className={tab === 'notifications' ? 'active' : ''} onClick={() => setTab('notifications')}>Notification Settings</button><button className={tab === 'roles' ? 'active' : ''} onClick={() => setTab('roles')}>Role Management</button></div><section className="dashboard-card settings-card settings-panel">{tab === 'profile' && <><CardHeader eyebrow="Account" title="User profile" /><label className="settings-field">Full name<input defaultValue={settings.profile.name} /></label><label className="settings-field">Work email<input defaultValue={settings.profile.email} type="email" /></label><label className="settings-field">Current role<input defaultValue={settings.profile.role} readOnly /></label></>}{tab === 'notifications' && <><CardHeader eyebrow="Routing" title="Notification settings" /><label className="toggle-row"><span><strong>Critical alerts</strong><small>Notify the on-call operator immediately</small></span><input type="checkbox" defaultChecked={settings.notifications.critical} /></label><label className="toggle-row"><span><strong>Daily digest</strong><small>Send a morning summary to administrators</small></span><input type="checkbox" defaultChecked={settings.notifications.digest} /></label><label className="toggle-row"><span><strong>Maintenance windows</strong><small>Suppress expected changes during windows</small></span><input type="checkbox" defaultChecked={settings.notifications.maintenance} /></label></>}{tab === 'roles' && <><CardHeader eyebrow="Access control" title="Role management" /><div className="role-list">{settings.roles.map((role, index) => <div key={role}><span className={`role-badge role-${index}`}>{role.slice(0, 1)}</span><span><strong>{role}</strong><small>{index === 0 ? 'Full platform administration' : index === 1 ? 'Monitor and respond to incidents' : 'Read-only monitoring access'}</small></span><ChevronDown size={15} /></div>)}</div></>}<button className="primary-button" onClick={() => setSaved(true)}>{saved ? 'Settings saved' : 'Save changes'} <Check size={16} /></button></section></div> }
