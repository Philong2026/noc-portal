...
const BRAND_LOGO_URL = "http://svtelecom.vn/wp-content/uploads/2025/10/logo-ngang-r.png"
...
function AppShell() {
  ...
  const { user, signOut } = useAuth(); const location = useLocation(); const navigate = useNavigate(); const displayName = user?.name || 'SVTELECOM NOC'; const displayRole = user?.role || 'Administrator'; const displayUsername = user?.username || 'svtelecom'
...
function MonitoringPage() {
  ...
  useEffect(() => {
    const loadDevices = async () => {
      const rawDevices = await safeAsyncCall(() => api.getDevices(), [])
...
function TopologyPage() {
  ...
  useEffect(() => {
    let active = true
    api.getAlerts().then((items) => { if (active) setAlerts(Array.isArray(items) ? items : []) })
    return () => { active = false }
  }, [])
  ...
  const selectedLastAlert = selectedDevice ? alerts.find((alert) => String(alert.device || "").toLowerCase() === String(selectedDevice.hostname || selectedDevice.name || "").toLowerCase()) : null
          ...
          <aside className="dashboard-card device-detail-panel">
            ...
            <div style={{ marginTop: '12px' }}>
              ...
              <div className="detail-note" style={{ marginTop: '12px' }}>
                <span className="auth-eyebrow">Last Alert</span>
                <p>{selectedLastAlert ? <><strong>{selectedLastAlert.title || 'Alert'}</strong>{selectedLastAlert.time ? ` · ${selectedLastAlert.time}` : ""}</> : 'No alerts recorded for this device.'}</p>
              </div>
